...
package net.mcreator.lilypad.entity;
...
import net.mcreator.lilypad.entity.renderer.AlphaSpiderRenderer;
import net.mcreator.lilypad.LilypadModElements;

@LilypadModElements.ModElement.Tag
public class AlphaSpiderEntity extends LilypadModElements.ModElement {
	public static EntityType entity = (EntityType.Builder.<CustomEntity>create(CustomEntity::new, EntityClassification.MONSTER)
			.setShouldReceiveVelocityUpdates(true).setTrackingRange(64).setUpdateInterval(3).setCustomClientFactory(CustomEntity::new)
			.size(1.4f, 0.9f)).build("alpha_spider").setRegistryName("alpha_spider");

	public AlphaSpiderEntity(LilypadModElements instance) {
		super(instance, 53);
		FMLJavaModLoadingContext.get().getModEventBus().register(new AlphaSpiderRenderer.ModelRegisterHandler());
	...
	public void initElements() {
		...
		elements.items.add(() -> new SpawnEggItem(entity, -15329770, -14871398, new Item.Properties().group(LilypadExtensionItemGroup.tab))
				.setRegistryName("alpha_spider_spawn_egg"));
	}

	@SubscribeEvent
	public void addFeatureToBiomes(BiomeLoadingEvent event) {
		boolean biomeCriteria = false;
		if (new ResourceLocation("lilypad:alpha_forest").equals(event.getName()))
			biomeCriteria = true;
		if (!biomeCriteria)
			return;
		event.getSpawns().getSpawner(EntityClassification.MONSTER).add(new MobSpawnInfo.Spawners(entity, 50, 1, 3));
	}
